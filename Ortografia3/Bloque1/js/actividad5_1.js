var ejercicios = ['mhou', 'garho', 'naomhu', 'rmighoa', 'spoithla','íooervvrh','hómoippaot','htrmraeniae'];
var palabras = ['humo', 'hogar', 'humano', 'hormiga', 'hospital','hervívoro','hipopótamo','herramienta'];
var usados = [];
var id_boton = "";
var pos = -1;
var resultado;
...
function califica() {
    ...
    for(var i=1; i<=ejercicios.length; i++){
        ...
        if(palabra_ingresada!=palabras[i-1]){
            bandera=false;
...
function bien() {          
    if (usados.length === palabras.length) {
